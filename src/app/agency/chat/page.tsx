...
const CashChatPage = () => {
    ...
    const { isLoading: isLoadingChannel } = useQuery({
        queryKey: ['agencyCountryChannel'],
        queryFn: async () => {
            console.log("📥 [CashChatPage] Récupération du canal d'agence...");
            const response = await api.get('/staff/chat/country-channel');
    ...
    const { data: agents = [], isLoading: isLoadingAgents } = useQuery<Agent[]>({
        queryKey: ['agents-by-country'],
        queryFn: async () => {
            console.log("📥 [CashChatPage] Récupération de la liste des agents...");
            const response = await api.get('/staff/chat/agents/by-country');
